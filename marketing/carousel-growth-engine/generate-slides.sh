#!/usr/bin/env bash
# generate-slides.sh — Phase 3: generate 6 coherent carousel slides.
#
...
# palette, typography, and aesthetic stay consistent.
#
# Usage:
#   ./generate-slides.sh <analysis.json> <run-dir> [learnings.json]
#
...
ANALYSIS="${1:?Usage: generate-slides.sh <analysis.json> <run-dir> [learnings.json]}"
...
# 1. Build prompts, caption, title.
echo "[slides] Building prompts ..." >&2
node "$HERE/build-prompts.js" "$ANALYSIS" "$RUN_DIR" "$LEARNINGS"

PROMPTS="$RUN_DIR/slide-prompts.json"

# Pull each slide's image prompt out of slide-prompts.json (no jq dependency).
read_prompt() {
  local idx="$1"
  node -e '
    const fs = require("fs");
