...
RUBRIC = (
    "You are a STRICT quality reviewer for a vertical Gulfio news-carousel slide. "
    "The slide must display this exact headline text: \"{expect}\".\n"
    "A small gold category tag reading \"{tag}\" near the top is EXPECTED and CORRECT — "
    "it is part of the brand design, NOT a defect.\n"
    "Fail the slide if ANY of these are true:\n"
    "1. The headline text is badly garbled, clearly misspelled, or not legible. "
    "(A minor dropped filler word like 'the'/'of' is acceptable; obvious gibberish is not.)\n"
    ...
    "3. Any font name, hex color code, or stray instruction/label word (e.g. 'FINAL PART', "
    "'HEADLINE') is rendered as visible text. The expected category tag \"{tag}\" does NOT count.\n"
    "4. Text is cut off at the image edges.\n"
    "5. There is readable text in the bottom ~20% of the image.\n"
    "6. Imagery is graphic, disrespectful, or politically charged.\n"
...
def main() -> None:
    ...
    ap.add_argument("--allow-tag", default="", help="category tag word that is allowed to appear")
    ...
    body = {
        "contents": [
            {
                "parts": [
                    {"text": RUBRIC.format(expect=args.expect, tag=getattr(args, "allow_tag", "") or "BREAKING")},
